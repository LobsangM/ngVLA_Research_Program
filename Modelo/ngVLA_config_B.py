...
# ==================================================
# FUNCIÓN PARA AGREGAR RUIDO CORRELACIONADO
# ==================================================
def add_correlated_noise(fits_file, beam_arcsec, noise_nJy):
    ...
    # =====================================
    # FWHM -> sigma usando relación gaussiana
    # =====================================
...
# ==================================================
# FUNCIÓN PARA DIBUJAR IMAGEN CON BEAM
# ==================================================
...
for entry in REDSHIFT_SFR_TABLE:
    ...
    print(f"\n{'='*55}")
    print(f"  ngVLA-B  z={z}  SFR={sfr} M☉/yr  [{entry['label']}]")
    print(f"{'='*55}\n")

    cfg = copy.deepcopy(config)
    cfg["radio"]["redshift"] = z
    cfg["sfr"]               = {"total_sfr": sfr}
    cfg["output_folder"]     = f"resultados/{entry['label']}/ngVLA_B"
    ...
    fits_file = os.path.join(
        f"resultados/{entry['label']}/ngVLA_B",
        f"convolucion_{beam}arcsec.fits"
    )

    final_fits = add_correlated_noise(fits_file, beam, noise_nJy)
