...
def add_correlated_noise(fits_file, beam_arcsec, noise_nJy):
    ...
    # Acotado al tamaño de la imagen: con beams muy anchos (Core array,
    # pixel scale nativo muy fino) 6*sigma puede superar por mucho las
    # dimensiones de la imagen, disparando un kernel de decenas de miles
    # de píxeles por lado (varios GB) y el zero-padding de fftconvolve.
    # La renormalización L2 de abajo mantiene std ≈ sigma_beam aunque el
    # kernel quede truncado.
    kernel_size = min(int(6 * sigma_kernel_pix), data.shape[0] // 2)
...
def plot_with_beam(fits_file):
    ...
    if levels:
        # data.max() > lvl solo garantiza que algún píxel alcanza el nivel,
        # no que forme un contorno cerrado trazable: un pico de 1-2 píxeles
        # aislados puede producir un path vacío, y el transform de WCSAxes
        # revienta con "Expected 2 world coordinates, got 0" (bug conocido
        # de astropy/aplpy con contornos vacíos). Si pasa, se omite el
        # contorno en vez de tumbar todo el run.
        try:
            f.show_contour(
                fits_file,
                levels=levels,
            ...
            print("Aviso: no se pudo dibujar contorno (nivel sin path cerrado):", e)
...
# config ngVLA B (ngECT Subarray: Main, synthesized beam @1.4GHz)
# Beam ancho/sensible, como config B en el resto del README.
beam      = 1.24
